...
SINT32  offset2_AppEOI(VOID);
VOID    offset2_AppDeinit(VOID);
SINT32  offset2_CfgRead(VOID);
SINT32  offset2_SviSrvInit(VOID);
VOID    offset2_SviSrvDeinit(VOID);

/* Functions: administration, to be called only from within this file */
MLOCAL VOID offset2_CfgInit(VOID);
...
OFFSET2_BASE_PARMS offset2_BaseParams;
...
REAL64 hostwfc_OFFSET2OK[1];
MLOCAL SVI_ADDR SA_hostwfc_OFFSET2OK;
...
MLOCAL TASK_PROPERTIES TaskProperties_aControl = {
    "aOFFSET2_Ctrl",                 /* unique task name, maximum length 14 */
...
MLOCAL SVI_GLOBVAR SviGlobVarList[] = {
    {"CycleCounter", SVI_F_INOUT | SVI_F_UINT32, sizeof(UINT32), (UINT32 *) & CycleCount, 0, NULL,
     NULL}
    ,
    {"ModuleVersion", SVI_F_OUT | SVI_F_STRING, sizeof(offset2_Version),
     (UINT32 *) offset2_Version, 0, NULL, NULL},
...
input_WS_Filtered_ms = hostwfc_INFLW2WSFiltered[0];
input_WDir_Filtered_deg = hostwfc_INFLW2WdirFiltered[0];
input_TI_Filtered = hostwfc_INFLW2TurbIntFiltered[0];
input_ShearExp_Filtered = hostwfc_INFLW2ShearExpFiltered[0];
input_inflowOK = hostwfc_INFLW2OK[0];
...
OFFSET_2_U.input_WS_Filtered_ms =  input_WS_Filtered_ms;
OFFSET_2_U.input_WDir_Filtered_deg =  input_WDir_Filtered_deg;
OFFSET_2_U.input_TI_Filtered =  input_TI_Filtered;
OFFSET_2_U.input_ShearExp_Filtered =  input_ShearExp_Filtered;
OFFSET_2_U.input_inflowOK =  input_inflowOK;
...
output_Offset_deg =  OFFSET_2_Y.output_Offset_deg;
output_OK =  OFFSET_2_Y.output_OK;

// MATLABCODEGEN: CloseField Output Variable Assignment

// MATLABCODEGEN: OpenField Assign ITF output Variables to HOST SVI

hostwfc_OFFSET2Offsetdeg[0] = output_Offset_deg;
hostwfc_OFFSET2OK[0] = output_OK;
...
SINT32 offset2_AppEOI(VOID)
{
    ...
    do
    {
        ...
        snprintf(offset2_ModuleInfoDesc, sizeof(offset2_ModuleInfoDesc), "TODO: set application specific module info string");
...
*        For general configuration data, offset2_CfgParams is being used.
*        Being called by offset2_CfgRead.
...
MLOCAL SINT32 Task_CfgRead(VOID)
{
    ...
    snprintf(section, sizeof(section), offset2_BaseParams.AppName);
    ...
    for (idx = 0; idx < NbOfTasks; idx++)
    {
        ...
        ret = pf_GetStrg(section, group, key, "", (CHAR *) & TmpStrg, sizeof(TmpStrg),
                         offset2_BaseParams.CfgLine, offset2_BaseParams.CfgFileName);
        ...
        else
        {
            ...
            {
                TaskList[idx]->Priority = offset2_BaseParams.DefaultPriority;
...
MLOCAL SINT32 Task_CreateAll(VOID)
{
    ...
    for (idx = 0; idx < NbOfTasks; idx++)
    {
        ...
        if (TaskList[idx]->WDogRatio > 0)
        {
            ...
            TaskList[idx]->WdogId = sys_WdogCreate(offset2_AppName, wdogtime_us);
        ...
        if (strlen(TaskList[idx]->Name) < 1)
            snprintf(TaskList[idx]->Name, sizeof(TaskList[idx]->Name), "a%s_%d", offset2_AppName, idx + 1);
        ...
        TaskList[idx]->TaskId = sys_TaskSpawn(offset2_AppName, TaskName,
...
MLOCAL VOID Task_DeleteAll(VOID)
{
    ...
    do
    {
        ...
        if (AllTasksQuitted)
        {
            if (offset2_BaseParams.DebugMode & APP_DBG_INFO1)
...
MLOCAL SINT32 Task_InitTiming_Sync(TASK_PROPERTIES * pTaskData)
{
    ...
    pTaskData->SyncSessionId = mio_StartSyncSession(offset2_AppName);
...
MLOCAL VOID Task_WaitCycle(TASK_PROPERTIES * pTaskData)
{
     ...
     */
    if ((offset2_ModState == RES_S_STOP) || (offset2_ModState == RES_S_EOI))
    {
         ...
         */
        semTake(offset2_StateSema, WAIT_FOREVER);
    }
}
...
MLOCAL VOID offset2_CfgInit(VOID)
{
    ...
    strncpy(offset2_BaseParams.CfgFileName, offset2_ProfileName, M_PATHLEN);

    /* Application name */
    strncpy(offset2_BaseParams.AppName, offset2_AppName, M_MODNAMELEN);

    /* Line number in configuration file (used as start line for searching) */
    offset2_BaseParams.CfgLine = offset2_CfgLine;

    /* Worker task priority from module base parameters (BaseParms) */
    offset2_BaseParams.DefaultPriority = offset2_AppPrio;

    /* Debug mode from module base parameters (BaseParms) */
    offset2_BaseParams.DebugMode = offset2_Debug;
...
SINT32 offset2_SviSrvInit(VOID)
{
    ...
    UINT32  i;
    CHAR    Func[] = "offset2_SviSrvInit";

    /* If there are any SVI variables to be exported */
    if (NbOfGlobVars)
    {
        /* Initialize SVI-handler */
        offset2_SviHandle = svi_Init(offset2_AppName, 0, 0);
        if (!offset2_SviHandle)
        {
    ...
    {
        ret = svi_AddGlobVar(offset2_SviHandle, SviGlobVarList[i].VarName,
...
/**
********************************************************************************
* @brief Frees SVI server resources according to offset2_SviSrvInit()
...
VOID offset2_SviSrvDeinit(VOID)
{
    ...
    if (svi_DeInit(offset2_SviHandle) < 0)
        LOG_E(0, "offset2_SviSrvDeinit", "Could not de-initialize SVI server");
...
if (svi_GetAddr(pSviLib_hostwfc, "OFFSET2_OK", &SA_hostwfc_OFFSET2OK, &SviFormat) != SVI_E_OK)
{
    LOG_W(0, Func, "Could not get address of value hostwfc_OFFSET2OK!");
    return (ERROR);
}

// MATLABCODEGEN: CloseField Get ITF SA Address

if (svi_GetAddr(pSviLib_hostwfc, "OFFSET2_Status", &SA_AppStatus, &SviFormat) != SVI_E_OK)
{
...
UINT32 hostwfc_INFLW2WSFiltered_size = sizeof(hostwfc_INFLW2WSFiltered);
UINT32 hostwfc_INFLW2WdirFiltered_size = sizeof(hostwfc_INFLW2WdirFiltered);
UINT32 hostwfc_INFLW2TurbIntFiltered_size = sizeof(hostwfc_INFLW2TurbIntFiltered);
UINT32 hostwfc_INFLW2ShearExpFiltered_size = sizeof(hostwfc_INFLW2ShearExpFiltered);
UINT32 hostwfc_INFLW2OK_size = sizeof(hostwfc_INFLW2OK);
...
ret = svi_GetBlk(pSviLib_hostwfc, SA_hostwfc_INFLW2WSFiltered, (UINT32*) &hostwfc_INFLW2WSFiltered,  &hostwfc_INFLW2WSFiltered_size);
if (ret != SVI_E_OK)
   LOG_W(0, "SviClt_Read", "Could not read value hostwfc_INFLW2WSFiltered!");
ret = svi_GetBlk(pSviLib_hostwfc, SA_hostwfc_INFLW2WdirFiltered, (UINT32*) &hostwfc_INFLW2WdirFiltered,  &hostwfc_INFLW2WdirFiltered_size);
if (ret != SVI_E_OK)
   LOG_W(0, "SviClt_Read", "Could not read value hostwfc_INFLW2WdirFiltered!");
ret = svi_GetBlk(pSviLib_hostwfc, SA_hostwfc_INFLW2TurbIntFiltered, (UINT32*) &hostwfc_INFLW2TurbIntFiltered,  &hostwfc_INFLW2TurbIntFiltered_size);
if (ret != SVI_E_OK)
   LOG_W(0, "SviClt_Read", "Could not read value hostwfc_INFLW2TurbIntFiltered!");
ret = svi_GetBlk(pSviLib_hostwfc, SA_hostwfc_INFLW2ShearExpFiltered, (UINT32*) &hostwfc_INFLW2ShearExpFiltered,  &hostwfc_INFLW2ShearExpFiltered_size);
if (ret != SVI_E_OK)
   LOG_W(0, "SviClt_Read", "Could not read value hostwfc_INFLW2ShearExpFiltered!");
ret = svi_GetBlk(pSviLib_hostwfc, SA_hostwfc_INFLW2OK, (UINT32*) &hostwfc_INFLW2OK,  &hostwfc_INFLW2OK_size);
if (ret != SVI_E_OK)
   LOG_W(0, "SviClt_Read", "Could not read value hostwfc_INFLW2OK!");
...
ret = svi_SetBlk(pSviLib_hostwfc, SA_hostwfc_OFFSET2Offsetdeg, (UINT32*) &hostwfc_OFFSET2Offsetdeg, sizeof(hostwfc_OFFSET2Offsetdeg) );
if (ret != SVI_E_OK)
     LOG_W(0, "SviClt_Write", "Could not read value hostwfc_OFFSET2Offsetdeg!");

ret = svi_SetBlk(pSviLib_hostwfc, SA_hostwfc_OFFSET2OK, (UINT32*) &hostwfc_OFFSET2OK, sizeof(hostwfc_OFFSET2OK) );
if (ret != SVI_E_OK)
     LOG_W(0, "SviClt_Write", "Could not read value hostwfc_OFFSET2OK!");
