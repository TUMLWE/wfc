...
#include "tumitfc.h"
#include "tumitfc_e.h"
#include "tumitfc_int.h"

/* Functions: administration, to be called from outside this file */
SINT32  tumitfc_AppEOI(VOID);
VOID    tumitfc_AppDeinit(VOID);
SINT32  tumitfc_CfgRead(VOID);
SINT32  tumitfc_SviSrvInit(VOID);
VOID    tumitfc_SviSrvDeinit(VOID);

/* Functions: administration, to be called only from within this file */
MLOCAL VOID tumitfc_CfgInit(VOID);
...
TUMITFC_BASE_PARMS tumitfc_BaseParams;
...
MLOCAL TASK_PROPERTIES TaskProperties_aControl = {
    "aTUMITFC_Ctrl",                 /* unique task name, maximum length 14 */
...
MLOCAL SVI_GLOBVAR SviGlobVarList[] = {
    {"CycleCounter", SVI_F_INOUT | SVI_F_UINT32, sizeof(UINT32), (UINT32 *) & CycleCount, 0, NULL,
     NULL}
    ,
    {"ModuleVersion", SVI_F_OUT | SVI_F_STRING, sizeof(tumitfc_Version),
     (UINT32 *) tumitfc_Version, 0, NULL, NULL}
};
...
SINT32 tumitfc_AppEOI(VOID)
{
    ...
    do
    {
        ...
        snprintf(tumitfc_ModuleInfoDesc, sizeof(tumitfc_ModuleInfoDesc), "TODO: set application specific module info string");
...
*        For general configuration data, tumitfc_CfgParams is being used.
*        Being called by tumitfc_CfgRead.
...
MLOCAL SINT32 Task_CfgRead(VOID)
{
    ...
    snprintf(section, sizeof(section), tumitfc_BaseParams.AppName);
    ...
    for (idx = 0; idx < NbOfTasks; idx++)
    {
        ...
        ret = pf_GetStrg(section, group, key, "", (CHAR *) & TmpStrg, sizeof(TmpStrg),
                         tumitfc_BaseParams.CfgLine, tumitfc_BaseParams.CfgFileName);
        ...
        else
        {
            ...
            {
                TaskList[idx]->Priority = tumitfc_BaseParams.DefaultPriority;
...
MLOCAL SINT32 Task_CreateAll(VOID)
{
    ...
    for (idx = 0; idx < NbOfTasks; idx++)
    {
        ...
        if (TaskList[idx]->WDogRatio > 0)
        {
            ...
            TaskList[idx]->WdogId = sys_WdogCreate(tumitfc_AppName, wdogtime_us);
        ...
        if (strlen(TaskList[idx]->Name) < 1)
            snprintf(TaskList[idx]->Name, sizeof(TaskList[idx]->Name), "a%s_%d", tumitfc_AppName, idx + 1);
        ...
        TaskList[idx]->TaskId = sys_TaskSpawn(tumitfc_AppName, TaskName,
...
MLOCAL VOID Task_DeleteAll(VOID)
{
    ...
    do
    {
        ...
        if (AllTasksQuitted)
        {
            if (tumitfc_BaseParams.DebugMode & APP_DBG_INFO1)
...
MLOCAL SINT32 Task_InitTiming_Sync(TASK_PROPERTIES * pTaskData)
{
    ...
    pTaskData->SyncSessionId = mio_StartSyncSession(tumitfc_AppName);
...
MLOCAL VOID Task_WaitCycle(TASK_PROPERTIES * pTaskData)
{
     ...
     */
    if ((tumitfc_ModState == RES_S_STOP) || (tumitfc_ModState == RES_S_EOI))
    {
         ...
         */
        semTake(tumitfc_StateSema, WAIT_FOREVER);
    }
}
...
MLOCAL VOID tumitfc_CfgInit(VOID)
{
    ...
    strncpy(tumitfc_BaseParams.CfgFileName, tumitfc_ProfileName, M_PATHLEN);

    /* Application name */
    strncpy(tumitfc_BaseParams.AppName, tumitfc_AppName, M_MODNAMELEN);

    /* Line number in configuration file (used as start line for searching) */
    tumitfc_BaseParams.CfgLine = tumitfc_CfgLine;

    /* Worker task priority from module base parameters (BaseParms) */
    tumitfc_BaseParams.DefaultPriority = tumitfc_AppPrio;

    /* Debug mode from module base parameters (BaseParms) */
    tumitfc_BaseParams.DebugMode = tumitfc_Debug;
...
SINT32 tumitfc_SviSrvInit(VOID)
{
    ...
    UINT32  i;
    CHAR    Func[] = "tumitfc_SviSrvInit";

    /* If there are any SVI variables to be exported */
    if (NbOfGlobVars)
    {
        /* Initialize SVI-handler */
        tumitfc_SviHandle = svi_Init(tumitfc_AppName, 0, 0);
        if (!tumitfc_SviHandle)
        {
    ...
    {
        ret = svi_AddGlobVar(tumitfc_SviHandle, SviGlobVarList[i].VarName,
...
/**
********************************************************************************
* @brief Frees SVI server resources according to tumitfc_SviSrvInit()
...
VOID tumitfc_SviSrvDeinit(VOID)
{
    ...
    if (svi_DeInit(tumitfc_SviHandle) < 0)
        LOG_E(0, "tumitfc_SviSrvDeinit", "Could not de-initialize SVI server");
