...
SINT32  inflw1_AppEOI(VOID);
VOID    inflw1_AppDeinit(VOID);
SINT32  inflw1_CfgRead(VOID);
SINT32  inflw1_SviSrvInit(VOID);
VOID    inflw1_SviSrvDeinit(VOID);

/* Functions: administration, to be called only from within this file */
MLOCAL VOID inflw1_CfgInit(VOID);
...
INFLW1_BASE_PARMS inflw1_BaseParams;
...
MLOCAL TASK_PROPERTIES TaskProperties_aControl = {
    "aINFLW1_Ctrl",                 /* unique task name, maximum length 14 */
...
MLOCAL SVI_GLOBVAR SviGlobVarList[] = {
    {"CycleCounter", SVI_F_INOUT | SVI_F_UINT32, sizeof(UINT32), (UINT32 *) & CycleCount, 0, NULL,
     NULL}
    ,
    {"ModuleVersion", SVI_F_OUT | SVI_F_STRING, sizeof(inflw1_Version),
     (UINT32 *) inflw1_Version, 0, NULL, NULL},
...
INFLW_1_U.input_WS_ms =  input_WS_ms;
INFLW_1_U.input_Dir_deg =  input_Dir_deg;
INFLW_1_U.input_WS_54m =  input_WS_54m;
...
output_WS =  INFLW_1_Y.output_WS;
output_WDir =  INFLW_1_Y.output_WDir;
output_TI =  INFLW_1_Y.output_TI;
output_ShearExp =  INFLW_1_Y.output_ShearExp;
output_InflowOK =  INFLW_1_Y.output_InflowOK;

// MATLABCODEGEN: CloseField Output Variable Assignment

// MATLABCODEGEN: OpenField Assign ITF output Variables to HOST SVI

hostwfc_INFLW1WSFiltered[0] = output_WS;
hostwfc_INFLW1WdirFiltered[0] = output_WDir;
hostwfc_INFLW1TurbIntFiltered[0] = output_TI;
hostwfc_INFLW1ShearExpFiltered[0] = output_ShearExp;
hostwfc_INFLW1OK[0] = output_InflowOK;
...
SINT32 inflw1_AppEOI(VOID)
{
    ...
    do
    {
        ...
        snprintf(inflw1_ModuleInfoDesc, sizeof(inflw1_ModuleInfoDesc), "TODO: set application specific module info string");
...
*        For general configuration data, inflw1_CfgParams is being used.
*        Being called by inflw1_CfgRead.
...
MLOCAL SINT32 Task_CfgRead(VOID)
{
    ...
    snprintf(section, sizeof(section), inflw1_BaseParams.AppName);
    ...
    for (idx = 0; idx < NbOfTasks; idx++)
    {
        ...
        ret = pf_GetStrg(section, group, key, "", (CHAR *) & TmpStrg, sizeof(TmpStrg),
                         inflw1_BaseParams.CfgLine, inflw1_BaseParams.CfgFileName);
        ...
        else
        {
            ...
            {
                TaskList[idx]->Priority = inflw1_BaseParams.DefaultPriority;
...
MLOCAL SINT32 Task_CreateAll(VOID)
{
    ...
    for (idx = 0; idx < NbOfTasks; idx++)
    {
        ...
        if (TaskList[idx]->WDogRatio > 0)
        {
            ...
            TaskList[idx]->WdogId = sys_WdogCreate(inflw1_AppName, wdogtime_us);
        ...
        if (strlen(TaskList[idx]->Name) < 1)
            snprintf(TaskList[idx]->Name, sizeof(TaskList[idx]->Name), "a%s_%d", inflw1_AppName, idx + 1);
        ...
        TaskList[idx]->TaskId = sys_TaskSpawn(inflw1_AppName, TaskName,
...
MLOCAL VOID Task_DeleteAll(VOID)
{
    ...
    do
    {
        ...
        if (AllTasksQuitted)
        {
            if (inflw1_BaseParams.DebugMode & APP_DBG_INFO1)
...
MLOCAL SINT32 Task_InitTiming_Sync(TASK_PROPERTIES * pTaskData)
{
    ...
    pTaskData->SyncSessionId = mio_StartSyncSession(inflw1_AppName);
...
MLOCAL VOID Task_WaitCycle(TASK_PROPERTIES * pTaskData)
{
     ...
     */
    if ((inflw1_ModState == RES_S_STOP) || (inflw1_ModState == RES_S_EOI))
    {
         ...
         */
        semTake(inflw1_StateSema, WAIT_FOREVER);
    }
}
...
MLOCAL VOID inflw1_CfgInit(VOID)
{
    ...
    strncpy(inflw1_BaseParams.CfgFileName, inflw1_ProfileName, M_PATHLEN);

    /* Application name */
    strncpy(inflw1_BaseParams.AppName, inflw1_AppName, M_MODNAMELEN);

    /* Line number in configuration file (used as start line for searching) */
    inflw1_BaseParams.CfgLine = inflw1_CfgLine;

    /* Worker task priority from module base parameters (BaseParms) */
    inflw1_BaseParams.DefaultPriority = inflw1_AppPrio;

    /* Debug mode from module base parameters (BaseParms) */
    inflw1_BaseParams.DebugMode = inflw1_Debug;
...
SINT32 inflw1_SviSrvInit(VOID)
{
    ...
    UINT32  i;
    CHAR    Func[] = "inflw1_SviSrvInit";

    /* If there are any SVI variables to be exported */
    if (NbOfGlobVars)
    {
        /* Initialize SVI-handler */
        inflw1_SviHandle = svi_Init(inflw1_AppName, 0, 0);
        if (!inflw1_SviHandle)
        {
    ...
    {
        ret = svi_AddGlobVar(inflw1_SviHandle, SviGlobVarList[i].VarName,
...
/**
********************************************************************************
* @brief Frees SVI server resources according to inflw1_SviSrvInit()
...
VOID inflw1_SviSrvDeinit(VOID)
{
    ...
    if (svi_DeInit(inflw1_SviHandle) < 0)
        LOG_E(0, "inflw1_SviSrvDeinit", "Could not de-initialize SVI server");
...
if (svi_GetAddr(pSviLib_hostwfc, "INFLW1_Status", &SA_AppStatus, &SviFormat) != SVI_E_OK)
{
...
ret = svi_SetBlk(pSviLib_hostwfc, SA_hostwfc_INFLW1WSFiltered, (UINT32*) &hostwfc_INFLW1WSFiltered, sizeof(hostwfc_INFLW1WSFiltered) );
if (ret != SVI_E_OK)
     LOG_W(0, "SviClt_Write", "Could not read value hostwfc_INFLW1WSFiltered!");

ret = svi_SetBlk(pSviLib_hostwfc, SA_hostwfc_INFLW1WdirFiltered, (UINT32*) &hostwfc_INFLW1WdirFiltered, sizeof(hostwfc_INFLW1WdirFiltered) );
if (ret != SVI_E_OK)
     LOG_W(0, "SviClt_Write", "Could not read value hostwfc_INFLW1WdirFiltered!");

ret = svi_SetBlk(pSviLib_hostwfc, SA_hostwfc_INFLW1TurbIntFiltered, (UINT32*) &hostwfc_INFLW1TurbIntFiltered, sizeof(hostwfc_INFLW1TurbIntFiltered) );
if (ret != SVI_E_OK)
     LOG_W(0, "SviClt_Write", "Could not read value hostwfc_INFLW1TurbIntFiltered!");

ret = svi_SetBlk(pSviLib_hostwfc, SA_hostwfc_INFLW1ShearExpFiltered, (UINT32*) &hostwfc_INFLW1ShearExpFiltered, sizeof(hostwfc_INFLW1ShearExpFiltered) );
if (ret != SVI_E_OK)
     LOG_W(0, "SviClt_Write", "Could not read value hostwfc_INFLW1ShearExpFiltered!");

ret = svi_SetBlk(pSviLib_hostwfc, SA_hostwfc_INFLW1OK, (UINT32*) &hostwfc_INFLW1OK, sizeof(hostwfc_INFLW1OK) );
if (ret != SVI_E_OK)
     LOG_W(0, "SviClt_Write", "Could not read value hostwfc_INFLW1OK!");
