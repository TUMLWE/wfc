...
#include "rtwtypes.h"
#include <string.h>
#include "OFFSET_2_private.h"

/* External inputs (root inport signals with default storage) */
ExtU_OFFSET_2_T OFFSET_2_U;

/* External outputs (root outports fed by signals with default storage) */
ExtY_OFFSET_2_T OFFSET_2_Y;

/* Real-time model */
static RT_MODEL_OFFSET_2_T OFFSET_2_M_;
RT_MODEL_OFFSET_2_T *const OFFSET_2_M = &OFFSET_2_M_;
...
void OFFSET_2_step(void)
{
   ...
   */
  rtb_AND = ((OFFSET_2_U.input_WS_Filtered_ms >= 3.0) &&
             (OFFSET_2_U.input_WS_Filtered_ms <= 12.0));
  ...
  output_OK_tmp = ((!(OFFSET_2_U.input_WDir_Filtered_deg >= 185.0)) ||
                   (!(OFFSET_2_U.input_WDir_Filtered_deg <= 260.0)));
  output_OK_tmp_0 = ((!(OFFSET_2_U.input_TI_Filtered >= 0.0)) ||
                     (!(OFFSET_2_U.input_TI_Filtered <= 0.4)));
  output_OK_tmp_1 = ((!(OFFSET_2_U.input_ShearExp_Filtered >= -0.99)) ||
                     (!(OFFSET_2_U.input_ShearExp_Filtered <= 0.99)));
   ...
   */
  OFFSET_2_Y.output_OK = (uint8_T)((((uint32_T)!rtb_AND << 1U | output_OK_tmp) <<
  ...
  if ((!rtb_AND) || output_OK_tmp || output_OK_tmp_0 || output_OK_tmp_1 ||
      (OFFSET_2_U.input_inflowOK > 0.0)) {
    /* Outport: '<Root>/output_Offset_deg' incorporates:
     *  Constant: '<Root>/Zero'
     */
    OFFSET_2_Y.output_Offset_deg = 0.0;
  } else {
     ...
     *  Inport: '<Root>/input_WS_Filtered_ms'
     */
    bpIndices[0U] = plook_binca(OFFSET_2_U.input_WS_Filtered_ms,
      OFFSET_2_ConstP.nDLookupTable_bp01Data, 6U, &frac);
    fractions[0U] = frac;
    bpIndices[1U] = plook_binca(OFFSET_2_U.input_WDir_Filtered_deg,
      OFFSET_2_ConstP.nDLookupTable_bp02Data, 40U, &frac);
    fractions[1U] = frac;
    bpIndices[2U] = plook_binca(OFFSET_2_U.input_TI_Filtered,
      OFFSET_2_ConstP.nDLookupTable_bp03Data, 2U, &frac);
    fractions[2U] = frac;
    bpIndices[3U] = plook_binca(OFFSET_2_U.input_ShearExp_Filtered,
      OFFSET_2_ConstP.nDLookupTable_bp04Data, 2U, &frac);
    fractions[3U] = frac;

    /* Outport: '<Root>/output_Offset_deg' incorporates:
     *  Lookup_n-D: '<S2>/n-D Lookup Table'
     */
    OFFSET_2_Y.output_Offset_deg = intrp4d_la_pw(bpIndices, fractions,
      OFFSET_2_ConstP.nDLookupTable_tableData,
      OFFSET_2_ConstP.nDLookupTable_dimSizes,
      OFFSET_2_ConstP.nDLookupTable_maxIndex);
  }
...
void OFFSET_2_initialize(void)
{
  /* Registration code */

  /* initialize error status */
  rtmSetErrorStatus(OFFSET_2_M, (NULL));

  /* external inputs */
  (void)memset(&OFFSET_2_U, 0, sizeof(ExtU_OFFSET_2_T));

  /* external outputs */
  (void)memset(&OFFSET_2_Y, 0, sizeof(ExtY_OFFSET_2_T));
}

/* Model terminate function */
void OFFSET_2_terminate(void)
{
